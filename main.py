...
_requests_pool = concurrent.futures.ThreadPoolExecutor(max_workers=5000)
...
async def catch_all_exception_handler(request, exc):
    ...
    return JSONResponse(
        status_code=500,
        content={"Gateway": "UNKNOWN", "Price": 0.0, "Response": f"ERROR: Internal server error", "Status": "error"}
    )
...
async def catch_all_middleware(request, call_next):
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"Gateway": "UNKNOWN", "Price": 0.0, "Response": "ERROR: Internal server error", "Status": "error"}
        )
    ...
    for attempt in range(5):
        try:
            ...
            if attempt < 4:
                await asyncio.sleep(1)
                continue
            return (False, f"ERROR: {last_error}", "UNKNOWN", "0.00", "USD")
    return (False, f"ERROR: {last_error}", "UNKNOWN", "0.00", "USD")
...
async def shopify_checker(
    ...
    try:
        try:
            ...
            raise HTTPException(status_code=400, detail={"error": str(e), "status": "error"})
        ...
        if clean_response in ('ORDER_PLACED', '3DS_REQUIRED'):
            status_value = True
        elif clean_response == 'CARD_DECLINED':
            status_value = False
        else:
            status_value = "error"
