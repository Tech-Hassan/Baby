...
@app.get("/")
async def root():
    return {"status": "ok", "service": "shopify-checker"}
...
async def process_card_async(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    for attempt in range(2):
        try:
            ...
            if success or ("Connector" not in message and "closed" not in message):
                return result
            last_error = message
        except Exception as e:
            last_error = str(e)
            if "Connector" in str(e) and "closed" in str(e) and attempt < 2:
                continue
            return (False, f"<b>Proxy Error: {last_error}</b>", "UNKNOWN", "0.00", "USD")
    return (False, f"<b>Proxy Error: {last_error}</b>", "UNKNOWN", "0.00", "USD")
...
async def shopify_checker(
    ...
    try:
        ...
        if success and clean_response in live_responses:
            status_value = True
        elif success:
            status_value = False
        else:
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:app", host="127.0.0.1", port=int(os.environ.get("PORT", 5002)), workers=1000)
